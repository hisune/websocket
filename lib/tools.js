...
{
    //console.log('-----');
    //console.log(data.length - protocol.start);
    var buffer = new Buffer(data.length - protocol.start);
...
exports.getSend = function(text)
{
    ...
    if (length > 65535) {
        buffer[1] = 0x7f; // 127
        // 8个字节长度
        buffer.writeUInt32BE(length >> 32, 2); // 高4字节
        buffer.writeUInt32BE(length & 4294967295, 6); // 低4字节
    } else if (length > 125) {
        buffer[1] = 0x7e; // 126
