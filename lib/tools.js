...
exports.getProtocol = function(data)
{
    ...
    if(protocol.payload_len >= 0 && protocol.payload_len <= 125){
        ...
        protocol.start += 8;
        protocol.len = (data[2] << 56) + (data[3] << 48) + (data[4] << 40) + (data[5] << 32) + (data[6] << 24) + (data[7] << 16) + (data[8] << 8) + data[9]; // 后64位，8字节
    }else{
        return false;
    }
    // 获取mask key
...
{
    protocol.buffer = new Buffer(data.length - protocol.start);
    for (var i = protocol.start, j = 0; i < data.length; i++, j++) {
        //对每个字节进行异或运算
        protocol.buffer[j] = data[i] ^ protocol.mask_key[j % 4];
    }

    protocol.len -= data.length - protocol.start; // 考虑分配情况，需要减去上次计算的数据长度
    protocol.start = 0; // 后面分片的数据开始未知为0
    protocol.msg += protocol.buffer.toString(); // msg的拼接
};
